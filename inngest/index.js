...
export const inngest = new Inngest({
  id: "mernstack-employeemanagementsystem",
});
...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    <p style="font-size: 16px;">EMS</p>
      ...
      // after 10 hours , mark attednace as checkiut qith status "LATE"
      await step.sleepUntil(
        "wait-for-the-1-hour",
...
//send email to admin , if admin does not take action on leave application with in 24 hours
...
//cron:check attendance at 11:30 A.M IST(06:00 UTC) and email absent employees
const attendanceReminderCron = inngest.createFunction(
  { id: "attendance-reminder-cron", triggers: [{ cron: "TZ=ASIA/Kolkata 30 11 * * *" }] },
  // 06:00 UTC = 11:30 AM IST
  async ({ event, step }) => {
    // step1: get todays'date range
    ...
    //step get employee IDS on approved leave today
